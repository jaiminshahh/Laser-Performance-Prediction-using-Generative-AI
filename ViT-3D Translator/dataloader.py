...
class SpatioTemporalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_pulse_data = torch.tensor(np.array(pd.read_csv(pls_inp, header=None))).squeeze(0)[35:]
        ...
        output_pulse_data = torch.tensor(np.array(pd.read_csv(pls_out, header=None))).squeeze(0)[35:]
        # output_pulse_data = output_pulse_data / torch.max(output_pulse_data)

        return input_3d_tensor.float(), output_image_data.float(), output_pulse_data.float()
...
def get_loader(config, fol_path, mode=None):
    ...
    if mode == 'train' or mode == 'valid':
        ...
        dataloader = DataLoader(dataset, batch_size=1,
                                num_workers=config['training_parameters']['num_workers'], shuffle=False)
